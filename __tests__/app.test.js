...
describe("NC games Error handling", () => {
    ...
    return request(app)
      ...
      .then(({ body }) => {
        expect(body.msg).toBe(
          `Patched object must have the form of { inc_votes: newVote } where newVote indicates the change in votes. Votes cannot go below 0`
        );
      });
  });
  ...
  test("STATUS 404, responds with a message when requested a invalid review ID OR request request for a review with no comments for /api.reviews/:review_id/comments", () => {
    return request(app)
      ...
      .then(({ body }) => {
        expect(body.msg).toBe(
          "Invalid review ID or this review has no comments"
        );
      });
  });
  ...
  test("DELETE /api/comments/:comment_id returns a error when comment id doesnt exist", () => {
    return request(app)
      .delete("/api/comments/999999")
      .expect(404)
      .then(({ body }) => {
        expect(body.msg).toBe("Comment id 999999 not found");
      });
  });
});
